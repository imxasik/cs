...
def _draw_header(ax, T, *, name, is_invest, obs_span, for_span, issued,
    ...
    ax.text(0.5, 0.24, obs_span + "   \u00b7   " + for_span,
            transform=ax.transAxes, fontsize=T["fs_subtitle"], fontweight="bold",
...
def _draw_footer(ax, T, left, right):
    ...
    ax.add_patch(Rectangle((0, 0.86), 0.16, 0.14, transform=ax.transAxes,
                           ...
                           clip_on=False))
    ax.text(0.008, 0.5, left, transform=ax.transAxes, fontsize=T["fs_footer"], fontweight="bold",
...
def _swatch(ax, x_c, y_c, kind, colour, fs, T, z=4):
    ...
    r = fs * 0.42
    if kind == "dot":
        _dot_patch(ax, x_c, y_c, r, fc=colour, ec="#000000", lw=0.9, z=z)
    elif kind == "ring":
        from matplotlib.patches import Ellipse
...
_KEY_SEC_GAP = 0.90     # clear space before a section title
_KEY_TITLE = 1.00       # the section-title band
_KEY_LEAD = 0.00        # lead-in between the title and the first item
_KEY_ITEM = 1.50        # legend item-row pitch
    ...
    while i < len(rows):
        if rows[i][0] == "section":
            ...
            for k, row in enumerate(items):
                ...
                label = _fit_text(label, col_w * aw - gut - 8, fs * 1.05)
...
_GLANCE_ROW = 3.20         # Generous row pitch for maximum breathing room
_GLANCE_VAL = 1.30         # Crisp, modern value font scaling


def _glance_card_height(stats, fs, T):
    rows = (len(stats) + 1) // 2
    # Expands overall card height to fit the padded rows comfortably
    return T["card_pad"] * 2.4 + fs * 2.0 + (rows * fs * _GLANCE_ROW)


def _draw_glance_card(ax, x0, x1, y1, stats, fs, T):
    """Spacious, clean dashboard summary card with clear row separation."""
    aw, ah = _ax_pt(ax)
    ...
    r_top = top + fs * 0.30 / ah
    r_bot = top - (rows - 1) * fs * _GLANCE_ROW / ah - fs * 2.20 / ah
    ax.plot([mid_x, mid_x], [r_bot, r_top], transform=ax.transAxes,
            color=T["card_edge_soft"], lw=1.0, zorder=3.6, clip_on=False)
    ...
    for i, (label, value, sub, colour) in enumerate(stats):
        ...
        # Label (Top line of cell)
        ax.text(cx, cy, label, transform=ax.transAxes,
                fontsize=fs * 0.82, fontweight="bold", color=T["ink_soft"],
                ha="left", va="center", zorder=4)
        
        # Value (Spaced neatly 1.20fs below label)
        ax.text(cx, cy - fs * 1.20 / ah, value, transform=ax.transAxes,
                fontsize=fs * _GLANCE_VAL, fontweight="bold", color=colour,
                ha="left", va="center", zorder=4)

        # Horizontal Hairline between rows (spaced well clear of values)
...
def _draw_states_and_labels(ax, T, bounds, geojson_path):
    """
    জুম আউট থাকলে শুধু প্রধান দেশগুলোর নাম (Country Labels) দেখাবে, 
    আর জুম ইন করলে স্টেট বর্ডার ও স্টেট লেবেল দেখাবে।
    """
    try:
    ...
    map_span = lon_max - lon_min  # ম্যাপের জুম উইন্ডো সাইজ

    # প্রধান দেশগুলোর তালিকা (Pakistan, Thailand, Sri Lanka সহ)
    major_countries = [
    ...
    # কন্ডিশন ১: ম্যাপ জুম আউট থাকলে শুধু দেশের নাম দেখাবে
    if map_span > 15.0:
        for country in major_countries:
            ...
            ax.text(
                ...
                fontsize=T["fs_tiny"] * 1.3,  # ত্রুটি সংশোধিত (fs_normal এর বদলে)
                fontweight="bold",
                color=T["ink"],
                ha="center", va="center",
                alpha=0.9,
    ...
    # কন্ডিশন ২: ম্যাপ জুম ইন হলে তখন স্টেট বর্ডার এবং স্টেট লেবেলগুলো দেখাবে
...
def _draw_map(ax, T, *, lon_min, lon_max, lat_min, lat_max, basemap_path,
    ...
    # ---- State / Province Borders & Labels ----
    if getattr(cfg, "SHOW_STATES", True) or getattr(cfg, "SHOW_STATE_LABELS", True):
        states_path = str(Path(basemap_path).with_name("states.geojson"))
        _draw_states_and_labels(ax, T, (lon_min, lon_max, lat_min, lat_max), states_path)
    ...
    ax.tick_params(axis="both", which="both", length=0, width=0, color=T["map_edge"], labelcolor=T["ink_soft"], labelsize=T["fs_tick"], pad=-2.0)
    ...
    ax.set_xticklabels([f"{v:.0f}\u00b0E" for v in ax.get_xticks()], va="bottom")
    ax.set_yticklabels([f"{v:.0f}\u00b0N" for v in ax.get_yticks()], ha="left", va="center")

    if show_grid:
        ax.grid(color=T["grid"], linestyle=(0, (3, 3)), linewidth=0.8,
                alpha=0.80, zorder=1)
    ...
    if track_obs is not None and len(track_obs):
        for lat, lon, wind in zip(track_obs["Latitude"],
                                  track_obs["Longitude"],
                                  track_obs["Intensity"]):
            ax.scatter(lon, lat, s=80, color=wind_color(wind),
                       edgecolor="#000000", linewidth=1.5, zorder=6)
    ...
    if track_for is not None and len(track_for):
        for lat, lon, wind in zip(track_for["Latitude"],
            ...
            ax.scatter(lon, lat, s=145, color=wind_color(wind),
                       edgecolor="#000000", linewidth=2.0, zorder=6.2)

    # ---- ports -------------------------------------------------------------
    if lon_span <= 10.0:
        for name, plat, plon, risk in visible_ports:
            ax.scatter(plon, plat, s=50, color=risk["color"],
                       edgecolor="#000000", linewidth=1.2, zorder=6)
    ...
    if landfall_info is not None:
        ax.plot(landfall_info["lon"], landfall_info["lat"], marker="*",
                markersize=16, color=T["landfall"],
...
def _scale_bar(ax, T, lon_span, lat_mid):
    ...
    x1_px = ax_bb.x1 - 0 * pt2px
    y0_px = ax_bb.y0 + 10 * pt2px
   ...
   # add_rrect(ax, bx0, by0, bx1, by1, 3.0, fc="#ffffff", ec=T["card_edge"],
    ...
    ax.add_patch(Rectangle((sx + seg, bar_y), seg, bar_h / ah_pt,
                           ...
                           edgecolor=T["ink"], linewidth=0.6, zorder=9,
                           clip_on=False))
    ax.text(sx + (bar_w / 2) / aw_pt, bar_y + bar_h / ah_pt + 5 / ah_pt, "KM",
            transform=ax.transAxes, fontsize=fs, fontweight="bold",
            color=T["ink_faint"], ha="center", va="center", zorder=9)
...
def _north_arrow(ax, T):
    ...
    ax.text(xa, ya + 2 / ah, "N", transform=ax.transAxes, fontsize=fs,
            ...
            zorder=9)
    # ---- Coastline & Border legend in Bottom-Left corner (Transparent - No Box) ----
    fs_bl = T["fs_tiny"] + 1.2
    
    # 1) Coastline
    ax.plot([0.025, 0.075], [0.055, 0.055], transform=ax.transAxes, 
            color=T["coast"], lw=2.2, zorder=9, clip_on=False)
    ax.text(0.085, 0.055, "Coastline", transform=ax.transAxes, 
            fontsize=fs_bl, fontweight="bold", color=T["ink_soft"], 
            ha="left", va="center", zorder=9)
    
    # 2) Country Border
    ax.plot([0.025, 0.075], [0.028, 0.028], transform=ax.transAxes, 
            color=T["border"], lw=1.8, linestyle=(0, (3.2, 2.2)), 
            zorder=9, clip_on=False)
    ax.text(0.085, 0.028, "Country Border", transform=ax.transAxes, 
            fontsize=fs_bl, fontweight="bold", color=T["ink_soft"], 
            ha="left", va="center", zorder=9)
...
def plot_cyclone(cyclone_name, track_data_obs, track_data_for, is_invest,
        ...
        if cfg.SHOW_PORTS:
            ...
            for band in PORT_RISK_BANDS:
                key_rows.append(("item", "dot",
                                 ...
                                 f"{band['label'].title()}"#\u00b7 " #f"{band['legend']}"
                                 ))
    ...
    # ---------------- glance stats (Storm Summary) --------------------------
    stats = []
    
    # ১ নম্বর সারি: বামে CURRENT, ডানে PRESSURE
    if has_obs:
    ...
    # ২ নম্বর সারি: বামে MAX OBSERVED, ডানে MAX FORECAST
    ...
    # ৩ নম্বর সারি: বামে MOTION NOW, ডানে MOTION NEXT
    ...
    # ৪ নম্বর সারি: বামে LANDFALL EST., ডানে ACE
    if cfg.SHOW_LANDFALL and landfall_info is not None:
    ...
    m_l, m_r = 0.020, 0.020      # left margin carries the °N tick labels
    m_t = m_b = 0.015
    header_h = 0.106
    ...
    if cfg.WIND_RADIUS_EXTENT:
        ...
        # buffer মান বাড়িয়ে Zoom Out করার জন্য অতিরিক্ত মার্জিন যোগ:
        extra_buf = max(0.0, cfg.BUFFER - 2.0)
        lat_min -= extra_buf
        lat_max += extra_buf
    else:
    ...
    # Format local time to 12-hour AM/PM without leading zero (e.g., 12PM, 23 SEP 2026)
    local_str = ""
    ...
    _draw_header(
        ...
        issued=(f"ISSUED {ci_tnd:%HZ, %d %b %Y}".upper() if ci_tnd is not None
                else "ISSUED --"),
    ...
    if ax_side is not None:
        ...
        fs_mult = {"glance": 1.38, "key": 1.20, "table": 1.0}
    ...
    def place_chip(text, lon, lat, fs, edge, tcol, candidates, lw=1.1,
        ...
        ax_map.annotate(
            ...
            bbox=dict(facecolor="#ffffff", alpha=alpha, edgecolor=edge,
                      linewidth=lw, boxstyle=f"round,pad={0.35:.2f}"))
        placed.append(box)
        return box
    ...
    if has_obs:
        #p_txt = "" if pressure is None or pd.isna(pressure) \
            #else f" \u00b7 {int(pressure)}MB"
        place_chip(f"NOW \u00b7 {int(ci)}KT",#{p_txt}",
                   prev_lon, prev_lat, fs_chip * 0.80, T["accent"], T["ink"],
        ...
        # 2) forecast point chips (Edge color matching intensity category)
    if has_for:
        for i in range(len(fc)):
            lat = float(fc["Latitude"].iloc[i])
            lon = float(fc["Longitude"].iloc[i])
            wind = fc["Intensity"].iloc[i]
            # T["card_edge"] এর জায়গায় wind_color(wind) বসানো হয়েছে
            place_chip(f"{fc['tnd'].iloc[i]:%d/%HZ} \u00b7 {int(wind)}KT",
                       lon, lat, fs_chip * 0.80, wind_color(wind), T["ink"],
                       ...
                       lw=1.4)  # আউটলাইন স্ট্রোক আরেকটু স্পষ্ট করতে lw=1.4 ব্যবহার করতে পারেন

                        
        # 3) landfall chip (controlled by SHOW_LANDFALL_LABEL config)
    ...
    # 4) port chips (জুম আউট করলে মার্কার ও লেবেল পুরোপুরি গায়েব হবে)
    if cfg.SHOW_PORTS and (lon_span <= 10.0):
        #allowed_ports = {r["name"] for r in approach_rows} if approach_rows else set()

        for name, plat, plon, band in visible_ports:
            #if allowed_ports and name not in allowed_ports:
                #continue
            ...
            ax_map.annotate(
                ...
                bbox=dict(facecolor="#ffffff", alpha=0.92,
                          edgecolor=band["color"], linewidth=1.1,
